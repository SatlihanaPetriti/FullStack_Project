...
const ProductForm = ({ show, onClose, product, onSave, allProducts = [] }) => {
    //1. Loading Categories (from localStorage) or default categories if none saved
    const [categories, setCategories] = useState(() => {
        const savedCategories = localStorage.getItem('productCategories');
        return savedCategories ? JSON.parse(savedCategories) : ["Indoor", "Outdoor", "Pet-Friendly"];
    });
    ...
    // 2. Saving Categories Automatically to localStorage whenever they change
    useEffect(() => {
    ...
    // Handle category selection including "Add New" option
    ...
    // 6. Add new category with duplicate prevention
    const addNewCategory = () => {
        ...
        if (existingCategory) {
            setCategoryError(`Category "${existingCategory}" already exists!`);
            return;
        }
                        ...
                        <Col md={4}>
                            <Form.Group className="mb-3">
                                ...
                                {!showNewCategoryInput ? (
                                    //3. Selecting a Category or choosing to add a new one
                                    <Form.Select
                                        name="category"
                                    ...
                                    // 5.Input for creating a new category  form.control
                                    <div>
                                        <div className="d-flex gap-2">
                                            ...
                                            <Button variant="secondary" onClick={cancelNewCategory}>
                                                Cancel
                                            </Button>
                                        </div>
                                        {categoryError && (
