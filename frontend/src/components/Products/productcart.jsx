...
const BASE_URL = `${import.meta.env.VITE_BACKEND_URL}/products/uploads/variants`;
...
const Productcart = () => {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      const firstImage = data?.variants?.find((v) => v.image)?.image || null;
      setProduct(data);
  ...
  const handleRemoveFromCart = () => removeFromCart(cartItem.id);
  ...
  return (
    <Container fluid className="product-cart-container">
      ...
      <Row className="gx-0 product-main-row">

        {/* KOLONA E IMAZHEVE */}
        <Col md={6} className="product-image-col">
          <Row className="gx-0">

            {/* Desktop: thumbnails vertikalisht majtas */}
            <Col xs={2} className="d-none d-md-flex flex-column gap-4 px-3">
              {product.variants?.map((variant) => (
                <Image
                  key={variant.id}
                  src={`${BASE_URL}/${variant.image}`}
                  roundedCircle
                  className={`product-thumb border ${selectedImage === variant.image ? "active-thumb" : ""}`}
            ...
            <Col xs={12} md={9}>
              ...
              {/* Mobile: thumbnails horizontalisht POSHTË fotos */}
              <div className="thumb-row-mobile">
                {product.variants?.map((variant) => (
                  <Image
                    key={variant.id}
                    src={`${BASE_URL}/${variant.image}`}
        ...
        <Col md={6} className="product-info d-flex flex-column gap-3">
          ...
          {cartItem ? (
            <Button size="lg" variant="danger" className="botton-cart mt-2" onClick={handleRemoveFromCart}>
              Remove from Cart
            </Button>
          ) : (
            <Button size="lg" className="botton-cart mt-2" disabled={product.stock <= 0} onClick={handleAddToCart}>
              Add To Cart
            </Button>
          )}
